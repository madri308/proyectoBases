...
AS   
	BEGIN
		...
		SET XACT_ABORT ON
			...
			DECLARE @idsRecibos TABLE (id INT IDENTITY(1,1), idRecibo INT,id_CC INT, Pagado INT, id_Prop INT)
			INSERT INTO @idsRecibos (idRecibo,id_CC,Pagado,id_Prop)
			SELECT id,id_CC,0,id_Propiedad
			FROM [dbo].[Recibos]
			WHERE estado = 0
			ORDER BY fecha ASC
			BEGIN TRAN
				...
				BEGIN
					...
					SET @idRecibo = (SELECT TOP 1 R.idRecibo
										FROM @idsRecibos R
										INNER JOIN @Pagos P ON P.idTipoRecibo = R.id_CC
										INNER JOIN [dbo].[Propiedad] PR ON PR.numFinca = P.numFinca AND PR.id = R.id_Prop
										WHERE P.id = @idMenor)
